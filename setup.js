...
if (process.argv[2] === '--run') {
  ...
  // Drain stdin before ending. Claude Code writes the session payload to every
  // hook, and exiting while that write is in flight breaks the pipe — which, in
  // the Stop hook, cost the turn its reply. Nothing is read here, so the stream
  // is simply consumed and discarded.
